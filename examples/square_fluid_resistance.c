...
int main(void) {
  ...
  while (is_open) {
    ...
    if (position->y >= liquid_y - obj_height) {
      Vector2D *drag = vector2d_clone(velocity);
      vector2d_mul(drag, -1);
      double mag = vector2d_mag(velocity);
      double speed = mag * mag;
      vector2d_set_mag(drag, (0.1) * rho * speed * frontal_surface *
                                 coefficient_of_drag);
      vector2d_add(acceleration, drag);
      vector2d_del(drag);
    }
    ...
    if (is_mouse_pressed && position->y < liquid_y - obj_height) {
